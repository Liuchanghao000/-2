package com.lch.goods.utils;

import com.github.pagehelper.PageInfo;

public class PageHelpUtil {
  public static String page(String url, PageInfo<?> pageInfo, String name) {
    String first = "<a href=" + url + "?page=1&name=" + name + ">首页</a>";
    if(pageInfo.getPrePage() == 0){
    	pageInfo.setPrePage(1);
    }
    String prev = "<a href=" + url + "?page=" + pageInfo.getPrePage() + "&name=" + name + ">上一页</a>";
    String pages = "";
    for(int i = 1; i <= pageInfo.getPages(); i++){
    	pages +="<a href=" + url + "?page=" + i + "&name=" + name + ">"+i+ " " + "</a>";
    }
    if(pageInfo.getNextPage() == 0){
    	pageInfo.setNextPage(pageInfo.getPages());
    }
    String next = "<a href=" + url + "?page=" + pageInfo.getNextPage() + "&name=" + name + ">下一页</a>";
    
    String last = "<a href=" + url + "?page=" + pageInfo.getLastPage() + "&name=" + name + ">尾页</a>";
